...
def VDDX_U1(vc,fc,fm,fp,fmm,fpp):
    return where(vc > 0., (vc)*((fc)-(fm)), (vc)*((fp)-(fc)) )
...
def VDDX_U4(vc,fc,fm,fp,fmm,fpp):
    where (vc > 0.0, (vc)*(4.*(fp) - 12.*(fm) + 2.*(fmm) + 6.*(fc))/12., 
...
def addzguard(fval,du):
    ...
    flarge = zeros(fval.shape + array([0,0,4,0]), fval.dtype)
...
#
    ...
    grad4D = [zeros(fval.shape, fval.dtype),  # r 
              zeros(fval.shape, fval.dtype),  # theta
              zeros(fval.shape, fval.dtype)]  # z
    ...
    grad4D[1][:,:,:,:] = (
            DDX_C4(fval_wg[:,:,2:-2,:], fval_wg[:,:,1:-3,:], fval_wg[:,:,3:-1,:], 
                                        fval_wg[:,:,0:-4,:], fval_wg[:,:,4:  ,:])
                                               / (du.dtheta*du.rxy[:,:,newaxis,newaxis]))
...
#
    ...
    res4D = zeros(vval[0].shape, vval[0].dtype)
    ...
    res4D[2:-2,:,:,:] = (
            DDX_C4(vval[0][2:-2,:,:,:], vval[0][1:-3,:,:,:], vval[0][3:-1,:,:,:], 
                                        vval[0][0:-4,:,:,:], vval[0][4:  ,:,:,:])
                                               / du.dr_n
         +
            vval[0][2:-2,:,:,:] / du.rxy[2:-2,:,newaxis,newaxis])
...
#
    ...
    advec4D = zeros(fval.shape, fval.dtype)
    ...
    if order == 'VDDX_U1':
        ...
        advec4D[2:-2,:,:,:] = (
            ...
            VDDX_U1(vval[1][2:-2,:,:,:],
                    ...
                    / (du.dtheta*du.rxy[2:-2,:,newaxis,newaxis])
            )
    ...
    rvrfval = vval[0]*fval*du.rxy[:,:,newaxis,newaxis]
    vthfval = vval[1]*fval
    ...
    if order == 'VDDX_L1':
        ...
        advec4D[2:-2,:,:,:] = (
            ...
            DDX_L1(rvrfval[2:-2,:,:,:], rvrfval[1:-3,:,:,:], rvrfval[3:-1,:,:,:], 
                    ...
                    / (du.dr_n*du.rxy[2:-2,:,newaxis,newaxis])
...
#
    ...
    res4D = zeros(fval.shape, fval.dtype)
...
#
    ...
    if order == 'D2DX2_C4':
        res4D[2:-2,:,:,:] = (
            ...
            +1./(du.rxy[2:-2,:,newaxis,newaxis])*DDX_C4(fval[2:-2,:,:,:], fval[1:-3,:,:,:], fval[3:-1,:,:,:], 
                                     fval[0:-4,:,:,:], fval[4:,:,:,:])/ (du.dr_n)
            + D2DX2_C4(fval_wg[2:-2,:,2:-2,:], fval_wg[2:-2,:,1:-3,:], fval_wg[2:-2,:,3:-1,:], 
                                        fval_wg[2:-2,:,0:-4,:], fval_wg[2:-2,:,4:  ,:])/ ((du.dtheta**2)*(du.rxy[2:-2,:,newaxis,newaxis]**2)))
...
def radial_Average(fval,du,order = 'trapezoid'):
    ...
    if order == 'trapezoid':
        #Trapezoid Rule
        res2D = zeros(fval.shape, fval.dtype)
