...
class BragFourVar(SymEqBase):

    def __init__(self, metric, te_fluc=True):
        self.te_fluc = te_fluc
    ...
    def build_symb_eq(self, p):
        ...
        if self.te_fluc:
            Te_eq = sympy.expand((
                                         ...
                                         + 1.71 * 2. / 3. * p.fTe * p.fv_par.diff(p.z)  # 1.71 2/3 Te Grad_par(vpar)
                                         + 2. / p.mu * p.nu_e * p.fTe  # 2 mu nu_e Te
                                 ) / p.eig_func)
        else:  # \frac{\partial T_e}{\partial t} = 0
            Te_eq = sympy.expand(p.fTe.diff(p.t) / p.eig_func)
