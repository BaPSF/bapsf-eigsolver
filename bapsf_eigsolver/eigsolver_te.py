...
def blendparams(p1, p2, frac, flog=False):
    ...
    if (p1.np==p2.np)and(p1.tp==p2.tp)and(p1.pp==p2.pp):
        ...
        p.grid = EqGrid(Nr=p1.Nr, np=p1.np, tp=p1.tp, pp=p1.pp, 
                        param=p1.param,
        ...
        p.grid  = EqGrid(Nr=p1.Nr)  # create new grid object  #TMP: add param argument
...
class SymbolicEq(object):
    ...
    def __init__(self, metric="cyl"):
        ...
        self.symb_eq = self.build_symb_eq(self.varpack)  # linear equations for all variables: N, vpar, phi, (Te if te_fluc==True)
    ...
    def build_symb_eq(self, p):
        ...
        Te_eq = (
            p.fTe.diff(p.t)
            + DotProd(p.vE, Grad(p.fTe, p.x, p.metric))
            ) / p.FExp
